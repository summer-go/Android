...
class EglHelper {
    ...
    public void initEgl(Surface surface, EGLContext eglContext) {
        ...
        int[] attrib_list = new int[] {
                ...
                EGL10.EGL_RENDERABLE_TYPE, mRenderType, // opengl版本2.0
                EGL10.EGL_NONE};

        int[] num_config = new int[1];
        // 1表示，最多只返回1个配置，num_config用来赋值->指定查询出来一共有多少组符合要求的配置
        // num_config传一个数组，实际上只需要第一个值，数组是模拟c++指针的作用，通过指针赋值
        // 第一次choose只是校验是否有符合要求的egl配置，不需要真正返回configs
        ...
        if (!mEgl.eglChooseConfig(mEglDisplay, attrib_list, configs,numConfigs, num_config)){
